...
data = data*450*3.1415926/180
# 计算相邻两个数值之间的差值
data_dot = data.diff().fillna(0)
data_dot = data_dot/0.002
# 计算data_dot相邻两个数的差值
data_ddot = data_dot.diff().fillna(0)
data_ddot = data_ddot/0.002
...
plt.plot(data_dot.index[:200], data_dot[0][:200], label='Data_dot')
...
t = np.arange(len(data_dot.index[:100]))
popt, pcov = curve_fit(fit_func, t, data_dot[0][:100])
print("a = ", popt[1]," k = ",popt[0])
t_fit = data_dot.index[:200]
